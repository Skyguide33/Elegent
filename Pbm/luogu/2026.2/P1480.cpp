...
private:
  ...
  BigInt(const vector<ull> &vct, int is_neg): _num(vct), _is_neg(is_neg) {} 

  BigInt(vector<ull> &&vct, int is_neg): _is_neg(is_neg) { _num = move(vct); }
