...
void push_up(int p, int pl, int pr, int *arr) {
  if(tag[p]) len[p] = arr[pr]-arr[pl];
  else if(pl+1 == pr) len[p] = 0;
  else len[p] = len[ls]+len[rs];
}

void update(int L, int R, int io, int p, int pl, int pr, int *arr) {
  if(L <= pl && pr <= R) {
    tag[p] += io;
...
int main() {
  ...
  int numr = unique(xx+1, xx+cnt+1)-(xx+1), numc = unique(yy+1, yy+cnt+1)-(yy+1);
  int ans = 0, pre;
  for(int i = 1; i <= cnt; ++i) {
    pre = len[1];
