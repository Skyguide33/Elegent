...
void Output(int L, int R) {
  ...
  for(it it1 = L_block; ; ++it1) {
    int a = (it1 == L_block? L: 0);
    int b = (it1 == R_block? R: it1->size());
...
void Delete(int L, int R) {
  it L_block = Find(L); Split(L_block, L); 
  it R_block = Find(R); Split(R_block, R);
  ++R_block;
...
int main() {
  ...
  while(n--) {
    ...
    if(opt[0] == 'M') scanf("%d", &pos);
      ...
      ch.clear(); scanf("%d", &len); ch.resize(len);
      for(int i = 0; i < len; ++i) {
        ch[i] = getchar();
        while(ch[i] < 32 || ch[i] > 126) ch[i] = getchar();
      }
      Insert(pos, ch);
    } else if(opt[0] == 'D') { scanf("%d", &len); Delete(pos, pos+len); }
    else if(opt[0] == 'G') { scanf("%d", &len); Output(pos, pos+len); }
    else if(opt[0] == 'P') --pos;
    else ++pos;
  }
  return 0;
