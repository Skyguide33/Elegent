...
bool nobalance(int u) {
  if((double)t[u].size*alpha <= (double)max(t[t[u].ls].size,t[t[u].rs].size)) return 1;
  return 0;
}
...
int main() {
  ...
  int q; cin >> q;
  // rebuild_num = 0; deep_timer = 0; max_deep = 0;
  while(q--) {
    int opt, x; cin >> opt >> x;
    switch(opt) {
    ...
    // cout << ">>" << endl; print_tree(root); cout << endl << "<<" << endl;
    // cnt_deep(root);
  }
  // cout << "rebuild num = " << rebuild_num << endl;
