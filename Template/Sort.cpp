...
void radix_sort(int *a, int *end) {
  ...
  while(max/exp) {
    int bucket[10] = {0};
    for(int i = 0; i < end-a; i++) bucket[(a[i]/exp)%10]++;
    for(int i = 1; i < 10; i++) bucket[i] += bucket[i-1];
    for(int i = end-a-1; i >= 0; i--) tmp[--bucket[(a[i]/exp)%10]] = a[i];
    for(int i = 0; i < end-a; i++) a[i] = tmp[i];
    exp *= 10;
  }
}
// 归并排序 3337856 
void Merge(int *a, int *mid, int *end) {
  ...
  while(a+i < mid) tmp[t++] = a[i++];
  while(a+j < end) tmp[t++] = a[j++];
...
void heap_sort(int *a, int *end) {
  for(int i = 0; i < end-a; i++) push(a[i]);
  for(int i = 0; i < end-a; i++) {
    a[i] = top(); pop();
  }
}
