...
public:
  ...
  explicit operator ll() const {
    ll res = 0;
    for(int i = (int)_num.size()-1; i >= 0; --i) res = res*10000+_num[i];
...
int main() {
  ...
  // cout << a << ' ' << b << endl;
  double srt = clock();
  BigInt ans = BigInt::fast_power(2, 1000000);
  double stop = clock();
  cout << "res: " << ans << endl;
  cout << "time: " << stop-srt << endl;
  return 0;
}
